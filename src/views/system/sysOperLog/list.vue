  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="businessType" label="业务类型">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.businessType==='INSERT'" type="success" effect="dark">新增</el-tag>
          <el-tag v-if="scope.row.businessType==='DELETE'" type="danger" effect="dark">删除</el-tag>
          <el-tag v-if="scope.row.businessType==='UPDATE'" type="warning" effect="dark">修改</el-tag>
          <el-tag v-if="scope.row.businessType==='SELECT'" type="info" effect="dark">查询</el-tag>
        </template>
      </el-table-column>
      <!-- <el-table-column prop="method" label="方法名称"/>-->
      <el-table-column prop="requestMethod" label="请求方式">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.requestMethod==='POST'" type="success" effect="dark">POST</el-tag>
          <el-tag v-if="scope.row.requestMethod==='DELETE'" type="danger" effect="dark">DELETE</el-tag>
          <el-tag v-if="scope.row.requestMethod==='UPDATE'" type="warning" effect="dark">UPDATE</el-tag>
          <el-tag v-if="scope.row.requestMethod==='SELECT'" type="info" effect="dark">SELECT</el-tag>
        </template>
      </el-table-column>
      <!--  0其他 1后台用户 2手机端用户    -->
      <el-table-column prop="operatorType" label="操作类别">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.operatorType==='OTHER'" type="info" effect="dark">其他</el-tag>
          <el-tag v-if="scope.row.operatorType==='MANAGE'" type="info" effect="dark">后台用户</el-tag>
          <el-tag v-if="scope.row.operatorType==='MOBILE'" type="info" effect="dark">手机端用户</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="operName" label="操作人员" />
      <!--<el-table-column prop="deptName" label="所属部门" />-->
      <el-table-column prop="operUrl" fit label="请求URL" />
    ...
    <el-dialog title="日志详情" :visible.sync="dialogVisible" :destroy-on-close="true" append-to-body width="45%">
      <el-form :model="operLogForm" label-width="150px" size="small" style="padding-right: 40px;">
        <el-input v-model="operLogForm.id" placeholder="ID">
          <template slot="prepend" disabled="inline-block">日志 &nbsp; - ID</template>
        </el-input>
        ...
        <el-input v-model="operLogForm.method">
          <template slot="prepend">方法名称</template>
        </el-input>
        ...
        <el-input v-model="operLogForm.operatorType">
          <template slot="prepend">操作类别</template>
        </el-input>
        ...
        <el-input v-model="operLogForm.operUrl">
          <template slot="prepend">请求URL</template>
        </el-input>
        <el-input v-model="operLogForm.operIp">
          <template slot="prepend">主机地址</template>
        </el-input>
        <el-input disabled placeholder="↓"><template slot="prepend">请求参数</template></el-input>
        <el-input v-model="operLogForm.operParam" type="textarea" autosize />
        <el-input disabled placeholder="↓"><template slot="prepend">返回参数</template></el-input>
        ...
        <el-input disabled placeholder="↓"><template slot="prepend">错误消息</template></el-input>
